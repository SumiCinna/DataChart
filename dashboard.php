...
<body class="min-h-screen bg-slate-950 text-slate-100 font-sans m-0 p-0">

  <!-- ── NAVBAR ── -->
  <header id="navbar" class="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/95 backdrop-blur-sm">
    <div class="max-w-6xl mx-auto px-7 py-3 flex items-center gap-3">
      ...
      <div class="ml-auto flex items-center gap-3">
        ...
        <a href="logout.php" class="border border-slate-700 hover:border-slate-500 text-slate-400 hover:text-slate-200 transition-colors text-xs rounded-md px-4 py-1.5">
          Sign out
        </a>
      </div>
    </div>
  ...
  <main class="max-w-6xl mx-auto px-7 py-8">
    ...
    <section id="no-file-screen" class="<?= $activeFile ? 'hidden' : '' ?>">
      <div class="text-center py-24">
        <div class="w-16 h-16 rounded-2xl border border-slate-700 bg-slate-800 flex items-center justify-center mx-auto mb-5">
        ...
        <h2 class="text-xl font-bold mb-2 text-slate-300">No active dataset</h2>
    ...
    <section id="dashboard-screen" class="<?= $activeFile ? '' : 'hidden' ?>">
      ...
      <!-- Controls bar -->
      <div class="bg-slate-800 rounded-lg border border-slate-700 p-3 mb-5 flex flex-wrap gap-4 items-center">
        <span class="text-slate-500 text-[11px] uppercase tracking-widest">Group / Label</span>
        <select id="label-col-select"
          class="bg-slate-950 border border-slate-700 text-slate-100 rounded-md px-3 py-1.5 text-xs focus:outline-none focus:border-brand-500">
        </select>
      ...
      <!-- No numeric warning -->
      <div id="no-numeric-warn" class="hidden bg-slate-800 border border-red-400/20 rounded-xl p-4 mb-5 text-red-300 text-sm">
          ...
          <div class="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
              <div class="flex items-center gap-2 flex-wrap">
                <span class="text-slate-100 font-semibold text-sm">{$label}</span>
                <span id="badge-entries-{$type}" class="bg-slate-950 text-slate-500 text-[10px] px-2 py-0.5 rounded-full border border-slate-700">— entries</span>
              ...
              <div class="flex gap-2">
                <button data-type="{$type}" data-action="filter"
                  class="filter-btn border border-slate-700 text-slate-500 hover:text-slate-200 hover:border-brand-500 transition-colors rounded-md px-3 py-1 text-[11px]">
                  Filter
                </button>
                <button data-type="{$type}" data-action="download"
                  class="dl-btn border border-slate-700 text-slate-500 hover:text-slate-200 transition-colors rounded-md px-3 py-1 text-[11px]">
                  PNG
                </button>
              </div>
            </div>
              ...
              <div class="flex flex-wrap gap-5 items-start">
                <div>
                  ...
                  <div class="flex gap-2 flex-wrap">
                    <select id="filter-col-{$type}" class="bg-slate-800 border border-slate-700 text-slate-100 rounded-md px-2 py-1 text-xs focus:outline-none">
                      <option value="">— none —</option>
                    </select>
                    <select id="filter-val-{$type}" class="hidden bg-slate-800 border border-brand-600 text-slate-100 rounded-md px-2 py-1 text-xs max-w-[180px] focus:outline-none">
